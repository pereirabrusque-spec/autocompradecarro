    ...
    try {
      ...
      if (provider === 'gemini') {
        ...
        if (data.promptFeedback?.blockReason || (data.candidates && data.candidates[0]?.finishReason === 'SAFETY')) {
           ...
           return res.json({ text: "Entendi. Estou analisando sua solicitação e já lhe darei a resposta correta. Posso ajudar com mais alguma dúvida sobre o veículo?", provider: 'gemini', model: modelName });
        }
