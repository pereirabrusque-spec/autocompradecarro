                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {key.id === currentInUseKeyId && (
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-black uppercase flex items-center gap-1 shadow-sm ${
                          key.status === 'ok' 
                            ? 'bg-blue-600 text-white animate-pulse shadow-blue-200' 
                            : 'bg-slate-400 text-white opacity-75'
                        }`}>
                          {key.status === 'ok' && <div className="w-1.5 h-1.5 bg-white rounded-full animate-ping" />}
                          Em Uso {key.status !== 'ok' && '(Tentando Reativar)'}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-3">
