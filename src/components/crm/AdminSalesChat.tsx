...
export const AdminSalesChat = React.memo(({ 
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white relative">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0 bg-slate-100 scroll-smooth" ref={chatContainerRef}>
        <AnimatePresence initial={false}>
          {(messages || []).map((m, index) => {
            const isClient = m.sender_id === conversationId;
            const msgTime = m.created_at ? format(new Date(m.created_at), 'HH:mm', { locale: ptBR }) : '';
            ...
            return (
              ...
              >
                <div className={`flex flex-col ${!isClient ? 'items-end' : 'items-start'} max-w-[85%]`}>
                  {/* Bubble */}
                  <div className={`p-3 rounded-2xl text-[14px] shadow-sm relative ${
                    isClient 
                      ? 'bg-white text-slate-800 rounded-bl-sm'
                      : (m.metadata?.from_ai || m.remetente === 'bot' || m.metadata?.ai_handled
                  ...
                  {/* Label (Opcional - para deixar claro quem mandou) */}
                  <span className={`text-[10px] mt-1 font-bold uppercase tracking-widest opacity-50 px-1`}>
                    {isClient ? 'Cliente' : (m.metadata?.from_ai || m.remetente === 'bot' || m.metadata?.ai_handled ? 'Agente IA' : 'Atendimento')}
                  </span>
                </div>
              </motion.div>
            );
