  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const leadPayload: any = {
        ...
        valor_ipva: parseFloat((formData.ipvaValue || '').replace(/[^\d,]/g, '').replace(',', '.')) || 0,
        valor_motor: parseFloat((formData.engineRepairValue || '').replace(/[^\d,]/g, '').replace(',', '.')) || 0,
        valor_cambio: parseFloat((formData.gearboxRepairValue || '').replace(/[^\d,]/g, '').replace(',', '.')) || 0,
        valor_pintura: parseFloat((formData.bodyRepairValue || '').replace(/[^\d,]/g, '').replace(',', '.')) || 0,
        problemas: problems,
